...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand(
    ...
    const convertJsonCommand = vscode.commands.registerCommand('rulewatcher.convertJson', async () => {
...
// Función para convertir JSON a Java
function convertJsonToJava(jsonObject: any, className: string): string {
  ...
  for (const key in jsonObject) {
    ...
    // Generar getter
    javaClass += `    public ${type} get${camelKey}() {\n`;
    javaClass += `        return ${key};\n`;
    javaClass += `    }\n\n`;

    // Generar setter
    javaClass += `    public void set${camelKey}(${type} ${key}) {\n`;
